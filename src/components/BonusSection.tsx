...
const BonusSection = () => {
  ...
  const planoMasterFeatures = [
    'TUDO DO PREMIUM',
    'VOZ INFINITA (VO3)',
    'IA CRIA CONTEÚDO INFINITO',
    'GERENCIA 10+ CONTAS TIKTOK',
    'GERENCIA 10+ CONTAS YOUTUBE',
    'SUPORTE VIP 24/7',
  ...
  return (
    <section id="bonus-section" className="py-20 bg-gradient-to-b from-purple-900 via-purple-800 to-purple-900">
      <div className="container mx-auto px-4 max-w-full overflow-hidden">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8 max-w-7xl mx-auto px-2">
              ...
              <div className="space-y-3 mb-8 relative z-10">
                ...
                <button 
                  onClick={() => {
                    ...
                    forceRedirectWithAllParams('https://pay.cakto.com.br/6krzq2x_576786');
                  }}
          ...
          {/* Plano Master - Novo plano */}
          <div className="relative transform hover:scale-105 transition-all duration-500">
            {/* Glowing border effect */}
            <div className="absolute -inset-1 bg-gradient-to-r from-red-500 via-red-600 to-red-700 rounded-2xl blur-sm animate-pulse"></div>
            
            {/* Master badge */}
            <div className="absolute -top-4 left-1/2 transform -translate-x-1/2 z-20">
              <div className="bg-gradient-to-r from-red-500 via-red-600 to-red-700 text-white px-3 md:px-6 py-1 md:py-2 rounded-full text-xs md:text-sm font-bold shadow-2xl border-2 border-red-400 animate-bounce whitespace-nowrap">
                <Crown className="w-4 h-4 inline mr-2" />
                PLANO MASTER
              </div>
            </div>
            ...
            {/* Side badges */}
            <div className="absolute -left-3 top-1/4 bg-orange-500 text-white px-3 py-1 rounded-full text-xs font-bold transform -rotate-12 animate-pulse shadow-lg">
              🔥 PRO
            </div>
            
            {/* Floating particles */}
            <div className="absolute -top-2 left-1/4 w-2 h-2 bg-red-400 rounded-full animate-bounce opacity-80"></div>
            <div className="absolute -top-1 right-1/4 w-1 h-1 bg-red-300 rounded-full animate-bounce delay-300 opacity-60"></div>
            <div className="absolute top-1/2 -left-1 w-1 h-1 bg-red-500 rounded-full animate-bounce delay-500 opacity-70"></div>
            <div className="absolute top-1/3 -right-1 w-2 h-2 bg-red-400 rounded-full animate-bounce delay-700 opacity-80"></div>
            
            <div className="relative bg-gradient-to-b from-red-800 via-red-700 to-red-900 rounded-2xl p-4 md:p-6 lg:p-8 border-2 border-red-400/50 shadow-2xl overflow-hidden">
              {/* Animated background pattern */}
              <div className="absolute inset-0 opacity-10">
                <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-br from-red-400/20 via-transparent to-red-600/20 animate-pulse"></div>
                <div className="absolute top-1/4 left-1/4 w-32 h-32 bg-red-400/10 rounded-full blur-xl animate-ping"></div>
                <div className="absolute bottom-1/4 right-1/4 w-24 h-24 bg-red-500/10 rounded-full blur-xl animate-ping delay-1000"></div>
              </div>
              
              {/* Shimmer overlay */}
              <div className="absolute inset-0 bg-gradient-to-r from-transparent via-red-400/10 to-transparent -skew-x-12 translate-x-[-200%] animate-shimmer"></div>
              
              <div className="text-center mb-8">
                <div className="flex items-center justify-center gap-2 mb-4">
                  <Crown className="w-6 h-6 text-red-400 animate-pulse" />
                  <h3 className="text-xl md:text-2xl lg:text-3xl font-bold bg-gradient-to-r from-red-400 via-red-300 to-red-500 bg-clip-text text-transparent">
                    Plano Master
                  </h3>
                  <Crown className="w-6 h-6 text-red-400 animate-pulse" />
                </div>
                <div className="mb-2">
                  <span className="text-red-400 line-through text-xl font-bold animate-pulse">R$ 67,00</span>
                </div>
                <div className="text-3xl md:text-4xl lg:text-5xl font-black bg-gradient-to-r from-red-400 via-red-300 to-red-500 bg-clip-text text-transparent mb-2 animate-pulse">
                  R$ 27,00
                </div>
                <div className="bg-gradient-to-r from-red-400 to-red-600 text-white px-3 md:px-4 py-1 md:py-2 rounded-full text-xs md:text-sm font-bold inline-block animate-bounce shadow-lg whitespace-nowrap">
                  💎 PLANO MAIS COMPLETO 💎
                </div>
              </div>

              <div className="space-y-3 mb-8 relative z-10">
                <div className="grid grid-cols-1 gap-y-3 mb-8 relative z-10">
                  {planoMasterFeatures.map((feature, index) => (
                    <div key={index} className="flex items-start gap-2">
                      <div className="flex-shrink-0 mt-0.5">
                        <div className="w-4 h-4 bg-gradient-to-r from-green-400 to-green-500 rounded-full flex items-center justify-center animate-pulse">
                          <Check className="w-2.5 h-2.5 text-white" />
                      ...
                      <span className="text-white text-sm font-medium hover:text-red-300 transition-colors">{feature}</span>
                    </div>
                  ))}
                </div>
                ...
                <button 
                  onClick={() => {
                    trackUTMifyEvent('plan_selection', { 
                      plan: 'master', 
                      price: '27,00',
                      button: 'quero_ser_master_agora'
                    });
                    // Force redirect with all URL parameters
                    forceRedirectWithAllParams('https://pay.cakto.com.br/urr9shv_576799');
                  }}
                  className="relative w-full bg-gradient-to-r from-red-400 via-red-500 to-red-600 text-white font-black py-3 md:py-4 lg:py-5 rounded-xl overflow-hidden group shadow-2xl text-base md:text-lg lg:text-xl transform hover:scale-110 transition-all duration-300 border-2 border-red-300"
                >
                  {/* Animated background gradient */}
                  <div className="absolute inset-0 bg-gradient-to-r from-red-300 via-red-400 to-red-500 opacity-0 group-hover:opacity-100 transition-opacity duration-300 animate-pulse"></div>
                  
                  {/* Shimmer effect */}
                  <div className="absolute inset-0 -skew-x-12 bg-gradient-to-r from-transparent via-white/50 to-transparent translate-x-[-200%] group-hover:translate-x-[200%] transition-transform duration-1000 animate-shimmer"></div>
                  
                  {/* Pulsing glow */}
                  <div className="absolute -inset-1 rounded-xl bg-red-400/60 blur-lg animate-ping"></div>
                  <div className="absolute -inset-2 rounded-xl bg-red-500/30 blur-xl animate-pulse"></div>
                  
                  {/* Multiple glow layers */}
                  <div className="absolute inset-0 rounded-xl bg-gradient-to-r from-red-400/40 via-red-500/40 to-red-600/40 animate-pulse"></div>
                  
                  {/* Button content */}
                  <span className="relative z-10 flex items-center justify-center gap-2 drop-shadow-lg">
                    <Crown className="w-5 h-5 animate-bounce" />
                    QUERO SER MASTER AGORA!
                    <Crown className="w-5 h-5 animate-bounce delay-300" />
                  </span>
                ...
                {/* Exclusive badge at bottom */}
                <div className="text-center mt-4">
                  <div className="inline-flex items-center gap-2 bg-gradient-to-r from-orange-500 to-orange-600 text-white px-3 md:px-4 py-1 md:py-2 rounded-full text-xs font-bold animate-pulse shadow-lg whitespace-nowrap">
                    🚨 PLANO MAIS COMPLETO 🚨
                  </div>
                </div>
              </div>
